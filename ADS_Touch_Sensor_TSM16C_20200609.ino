...
#define Cal_Con0 0x24 //Calibration Speeed Control (normal_up_cnt_bf / normal_down_cnt_bf)
#define Cal_Con1 0x25 //Calibration Speeed Control (normal_up_cnt_bs / normal_down_cnt_bs)
#define Cal_Con2 0x26 //Calibration Speeed Control (half_up_cnt_bf / half_down_cnt_bf)
#define Cal_Con3 0x27 //Calibration Speeed Control (half_up_cnt_bs / half_down_cnt_bs)
...
// ---------------------------------------------------------------------
   ...
   //------------ Hidden Register ----------------------------------------
   Wire.beginTransmission(TSM16C_SLAVE_GND); // sned ic slave address
   Wire.write(byte(Error_Percent));          // sends register address
   Wire.write(byte(0x0C));                   // sends register data
   Wire.endTransmission();                   // stop 

   Wire.beginTransmission(TSM16C_SLAVE_GND); // sned ic slave address
   Wire.write(byte(Safe_Percent));           // sends register address
   Wire.write(byte(0x08));                   // sends register data
   Wire.endTransmission();                   // stop 

   Wire.beginTransmission(TSM16C_SLAVE_GND); // sned ic slave address
   Wire.write(byte(Cal_Con0));               // sends register address
   Wire.write(byte(0x1F));                   // sends register data
   Wire.endTransmission();                   // stop 

   Wire.beginTransmission(TSM16C_SLAVE_GND); // sned ic slave address
   Wire.write(byte(Cal_Con1));               // sends register address
   Wire.write(byte(0x14));                   // sends register data
   Wire.endTransmission();                   // stop 

   Wire.beginTransmission(TSM16C_SLAVE_GND); // sned ic slave address
   Wire.write(byte(Cal_Con2));               // sends register address
   Wire.write(byte(0x14));                   // sends register data
   Wire.endTransmission();                   // stop 

   Wire.beginTransmission(TSM16C_SLAVE_GND); // sned ic slave address
   Wire.write(byte(Cal_Con3));               // sends register address
   Wire.write(byte(0x1F));                   // sends register data
   Wire.endTransmission();                   // stop 
